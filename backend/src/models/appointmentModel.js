...
async function findAll(userId, dataInicio, dataFim, status, patientId) {
  let query = `
  ...
  if (dataInicio) {
    ...
    query += ` AND a.scheduled_at >= $${params.length}`;
  }
  if (dataFim) {
...
async function findProximas(userId, limite) {
  const r = await pool.query(
    `SELECT a.*, p.name AS patient_name, p.funil_etapa
     FROM appointments a
     JOIN patients p ON p.id = a.patient_id
     WHERE a.user_id=$1 AND a.status='scheduled' AND a.scheduled_at >= NOW() AT TIME ZONE 'America/Sao_Paulo'
     ORDER BY a.scheduled_at ASC
     LIMIT $2`,
    [userId, limite || 5]
  );
  ...
  const r = await pool.query(
    `SELECT a.*, p.name AS patient_name, p.user_id
     ...
     WHERE a.user_id=$1
       ...
       AND a.scheduled_at <= (NOW() AT TIME ZONE 'America/Sao_Paulo') - INTERVAL '1 hour'
...
async function countsByStatus(userId) {
  const r = await pool.query(
    `SELECT
       ...
       COUNT(*) FILTER (WHERE status='scheduled' AND scheduled_at::date = (NOW() AT TIME ZONE 'America/Sao_Paulo')::date) AS hoje,
       COUNT(*) FILTER (WHERE status='scheduled'
         AND scheduled_at >= date_trunc('week', NOW() AT TIME ZONE 'America/Sao_Paulo')
         AND scheduled_at <  date_trunc('week', NOW() AT TIME ZONE 'America/Sao_Paulo') + INTERVAL '7 days') AS semana
     FROM appointments
     WHERE user_id=$1
...
async function countsPorMes(userId) {
  const r = await pool.query(
    `SELECT
     ...
     FROM appointments
     WHERE user_id=$1
       AND scheduled_at >= (NOW() AT TIME ZONE 'America/Sao_Paulo') - INTERVAL '6 months'
