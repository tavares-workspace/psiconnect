...
// Formata data no padrão que o Google Calendar entende como horário local
// Sem o "Z" no final, o Google respeita o timeZone informado
function formatarParaCalendar(date) {
  const d = new Date(date);
  const offset = -180; // UTC-3 Brasília em minutos
...
async function criarEvento(userId, consulta) {
  try {
    ...
    // scheduled_at vem como string pura "2026-05-27T11:00:00" sem timezone
    // Adiciona -03:00 para que o Node interprete como horário de Brasília
    const scheduledStr = consulta.scheduled_at.toString().slice(0, 19);
    const inicio = new Date(scheduledStr + '-03:00');
    ...
    const resource = {
      ...
      start: { dateTime: formatarParaCalendar(inicio), timeZone: 'America/Sao_Paulo' },
      end:   { dateTime: formatarParaCalendar(fim),   timeZone: 'America/Sao_Paulo' },
      conferenceData: {
        createRequest: {
